...
const CREDENTIALS = {
  email: ["kobita@masu.com", "bal@chal.com"],
  password: ["fuckMeDaddy", "123456"],
};
...
const AdminLogin = () => {
  ...
  useEffect(() => {
    if (localStorage.getItem(AUTH_KEY)) navigate("/dashboard");
  }, [navigate]);
  ...
  const onSubmit = (data: LoginForm) => {
    if (isValidCredential(data.email, data.password)) {
      localStorage.setItem(AUTH_KEY, btoa(`${data.email}:${Date.now()}`));
